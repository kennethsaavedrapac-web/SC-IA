// ──────────────────────────────────────────────────────────────────────────────
// DATASET ACTIVO: solo departamentos con coordenadas GPS reales en la BD MINSA.
// Los archivos JSON originales de todos los departamentos se mantienen en disco
// como fuente de verdad, pero el frontend solo carga los que tienen GPS válido.
// Carazo: 12 registros con GPS · Granada: 37 registros con GPS
// ──────────────────────────────────────────────────────────────────────────────
...
// Solo los departamentos que tienen coordenadas GPS reales en el dataset MINSA
...
// Bounds de Nicaragua usados para convertir coordenadas GPS a posición en mapa legacy (lat/lng %)
...
// Solo cargamos las unidades MINSA que tienen coordenadas GPS reales.
// Las 1,552 unidades sin GPS son excluidas del dataset activo del frontend;
// sus archivos JSON originales permanecen intactos en src/data/healthUnits/
// y pueden reintegrarse en el futuro cuando obtengan coordenadas.
const BASE_HEALTH_CENTERS: HealthCenter[] = HEALTH_UNIT_DATABASE.flatMap((department) =>
  department.unidades_salud
    .filter((unit) => typeof unit.latitud === "number" && typeof unit.longitud === "number")
    .map((unit, unitIndex) => {
      ...
      // Convertir coordenadas GPS reales a posición porcentual legacy (para compatibilidad)
      const top = clamp(((NICARAGUA_BOUNDS.north - unit.latitud!) / (NICARAGUA_BOUNDS.north - NICARAGUA_BOUNDS.south)) * 100, 6, 94);
      const left = clamp(((unit.longitud! - NICARAGUA_BOUNDS.west) / (NICARAGUA_BOUNDS.east - NICARAGUA_BOUNDS.west)) * 100, 6, 94);
      ...
      return {
        ...
        lat: top,
        lng: left,
        latitude: unit.latitud!,
        longitude: unit.longitud!,
